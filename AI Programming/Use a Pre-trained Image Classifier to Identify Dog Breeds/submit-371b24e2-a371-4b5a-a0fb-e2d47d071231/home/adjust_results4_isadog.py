...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    with open(dogfile, "r") as infile:
        ...
        while line != "":
            ...
            # Some lines have multiple names seperate with commas
            for name in names:
                if name not in dognames_dic:
                    dognames_dic[name] = 1
                    # Please note that the most classifier labels are composed (something like "basset, basset hound",
                    # "boston bull, boston terrier", and so on), but you are only saving the individual names to the 
                    # dognames_dic. This means that the classifier label "boston bull, boston terrier" will be marked 
                    # as a not dog when it should actually be marked as a dog.
